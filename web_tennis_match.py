        ...
        for court_idx in range(num_courts):
            if court_idx < len(round_matches):
                ...
                if st.session_state.match_type == "단식":
                    p1, p2 = match
                    row.append(f"{p1} vs {p2}")
                else:
                    team1, team2 = match
                    row.append(f"{' + '.join(team1)} vs {' + '.join(team2)}")
            else:
                row.append("-")
        rows.append(row)

    styled_table = pd.DataFrame(rows, columns=header)
